...
describe Emarsys::Event do
  ...
  describe ".trigger" do
    ...
    it "requests event trigger with additional attachment" do
      stub = stub_request(:post, "https://api.emarsys.net/api/v2/event/123/trigger").with(:body => {'key_id' => 3, 'external_id' => "jane.doe@example.com", attachment: [ { filename: 'test_attachment.pdf', data: 'Something' } ]}.to_json).to_return(standard_return_body)
      Emarsys::Event.trigger(123, key_id: 3, external_id: 'jane.doe@example.com', attachment: [ { filename: 'test_attachment.pdf', data: 'Something' } ])
      expect(stub).to have_been_requested.once
    end
  end
